...
static NSString *GDataFakeQNameForURLAndName(NSString *theURL, NSString *name) {
...
// be searching for a whole URL shoved in as a prefix, like
//   {http://foo}:name
...
+ (GDataXMLElement *)elementWithName:(NSString *)name URL:(NSString *)theURL {

  // since we don't know a prefix yet, shove in the whole URL; we'll look for
  // a proper namespace ptr later when addChild calls fixUpNamespacesForNode

  NSString *fakeQName = GDataFakeQNameForURLAndName(theURL, name);
...
+ (id)attributeWithName:(NSString *)name URL:(NSString *)attributeURL stringValue:(NSString *)value {

  // since we don't know a prefix yet, shove in the whole URL; we'll look for
  // a proper namespace ptr later when addChild calls fixUpNamespacesForNode

  NSString *fakeQName = GDataFakeQNameForURLAndName(attributeURL, name);
...
- (void)setStringValue:(NSString *)str {
  if (xmlNode_ != NULL && str != nil) {
    ...
    if (xmlNode_->type == XML_NAMESPACE_DECL) {
      ...
      // for a namespace node, the value is the namespace URL
...
- (NSArray *)nodesForXPath:(NSString *)xpath
  ...
  if (xmlNode_ != NULL && xmlNode_->doc != NULL) {
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        // the dictionary keys are prefixes; the values are URLs
        for (NSString *prefix in namespaces) {
          NSString *URL = [namespaces objectForKey:prefix];

          xmlChar *prefixChars = (xmlChar *) [prefix UTF8String];
          xmlChar *URLChars = (xmlChar *) [URL UTF8String];
          int result = xmlXPathRegisterNs(xpathCtx, prefixChars, URLChars);
          if (result != 0) {
#if DEBUG
...
- (NSArray *)namespaces {
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      // add this prefix/URL to the list, unless it's the implicit xml prefix
  ...
  if (xmlNode_ != NULL) {
    ...
    if (prefix) {
      ...
      if (foundNS) {
        ...
        // we found a namespace; fall back on elementsForLocalName:URL:
        // to get the elements
        NSString *desiredURL = [self stringFromXMLString:(foundNS->href)];
        NSString *localName = [[self class] localNameForName:desiredName];

        NSArray *nsArray = [self elementsForLocalName:localName URL:desiredURL];
        return nsArray;
      }
    }
...
- (NSArray *)elementsForLocalName:(NSString *)localName URL:(NSString *)URL {

  NSMutableArray *array = nil;

  if (xmlNode_ != NULL && xmlNode_->children != NULL) {

    xmlChar* desiredNSHref = GDataGetXMLString(URL);
    xmlChar* requestedLocalName = GDataGetXMLString(localName);
    xmlChar* expectedLocalName = requestedLocalName;

    // resolve the URL at the parent level, since usually children won't
    ...
    if (foundParentNS == NULL) {
      NSString *fakeQName = GDataFakeQNameForURLAndName(URL, localName);
  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr == NULL) {
      ...
      if (prefix) {
        ...
        // find the namespace for this prefix, and search on its URL to find
        // the xmlNsPtr
        name = [[self class] localNameForName:name];
        ns = xmlSearchNs(xmlNode_->doc, xmlNode_, GDataGetXMLString(prefix));
      }

      const xmlChar* nsURL = ((ns != NULL) ? ns->href : NULL);
      attrPtr = xmlHasNsProp(xmlNode_, GDataGetXMLString(name), nsURL);
    }
...
- (GDataXMLNode *)attributeForLocalName:(NSString *)localName
                                    URL:(NSString *)attributeURL {

  if (xmlNode_ != NULL) {

    const xmlChar* name = GDataGetXMLString(localName);
    const xmlChar* nsURL = GDataGetXMLString(attributeURL);

    xmlAttrPtr attrPtr = xmlHasNsProp(xmlNode_, name, nsURL);

    if (attrPtr == NULL) {
      // if the attribute is in a tree lacking the proper namespace,
      // the local name may include the full URL as a prefix
      NSString *fakeQName = GDataFakeQNameForURLAndName(attributeURL, localName);
...
- (NSString *)resolvePrefixForNamespaceURL:(NSString *)namespaceURL {

  if (xmlNode_ != NULL) {

    xmlChar* desiredNSHref = GDataGetXMLString(namespaceURL);
...
+ (void)fixQualifiedNamesForNode:(xmlNodePtr)nodeToFix
  ...
  //
  // The prefix may either be normal, "gd:foo", or a URL
  // "{http://blah.com/}:foo"
  ...
  if (nodeToFix->ns == NULL) {
    ...
    if (localName != NULL) {
      if (prefix != NULL) {
        ...
        // if the prefix is wrapped by { and } then it's a URL
        int prefixLen = xmlStrlen(prefix);
        if (prefixLen > 2
            && prefix[0] == '{'
            && prefix[prefixLen - 1] == '}') {

          // search for the namespace by URL
          xmlChar* URL = xmlStrsub(prefix, 1, prefixLen - 2);

          if (URL != NULL) {
            foundNS = xmlSearchNsByHref(graftPointNode->doc, graftPointNode, URL);
...
+ (void)fixDuplicateNamespacesForNode:(xmlNodePtr)nodeToFix
  ...
  if (nodeToFix->type == XML_ELEMENT_NODE) {
    ...
    while (definedNS != NULL) {
      ...
      // with both the same URL and the same prefix; if so, add a mapping for
      // it
