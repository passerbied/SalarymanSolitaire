...
@protected
  // NSXMLNodes can have a namespace URL or prefix even if not part
  // of a tree; xmlNodes cannot.  When we create nodes apart from
  // a tree, we'll store the dangling prefix or URL in the xmlNode's name,
  // like
  //   "prefix:name"
  // or
  //   "{http://URL}:name"
...
+ (GDataXMLElement *)elementWithName:(NSString *)name URL:(NSString *)value;
...
// namespace dictionary (keys are prefixes, values are URLs).
...
- (GDataXMLNode *)attributeForLocalName:(NSString *)name URL:(NSString *)attributeURL;
